...
class StudentDBHelper(context: Context) : SQLiteOpenHelper(
    ...
    fun searchStudentByName(name: String): ArrayList<StudentModel> {
        ...
        cursor?.use {
            while (it.moveToNext()) {
                ...
                val studentName = it.getString(it.getColumnIndexOrThrow(DBContract.UserEntry.COLUMN_NAME))
                val age = it.getString(it.getColumnIndexOrThrow(DBContract.UserEntry.COLUMN_AGE))
    ...
    fun readStudents(): ArrayList<StudentModel> {
        ...
        val db = writableDatabase
        val cursor: Cursor? = try {
            db.rawQuery("SELECT * FROM ${DBContract.UserEntry.TABLE_NAME} ORDER BY ${DBContract.UserEntry.COLUMN_NIM}", null)
        } catch (e: SQLiteException) {
            db.execSQL(SQL_CREATE_ENTRIES)
            return ArrayList()
        }
